...
function parseAssertionResult(assertionResultStr) {
  var assertionResult = JSON.parse(atob(assertionResultStr));

  var idp = assertionResult.idp;

  var assertion = JSON.parse(assertionResult.assertion);

  return { idp: idp, assertion: assertion };
}
...
function assert_rtcerror_rejection(errorDetail, promise, desc) {
  return promise.then(function (res) {
    ...
    assert_true(err instanceof RTCError, 'Expect error object to be instance of RTCError');

    assert_equals(err.errorDetail, errorDetail, 'Expect RTCError object have errorDetail set to ' + errorDetail);
