...
function validateStatsReport(statsReport) {
  ...
  try {
    for (var _iterator = statsReport.entries()[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var _step$value = _slicedToArray(_step.value, 2),
          id = _step$value[0],
          stats = _step$value[1];

      assert_equals(stats.id, id, 'expect stats.id to be the same as the key in statsReport');

      var validator = statsValidatorTable[stats.type];
      if (validator) {
...
function assert_stats_report_has_stats(statsReport, statsTypes) {
  var hasTypes = new Set([].concat(_toConsumableArray(statsReport.values())).map(function (stats) {
    return stats.type;
  }));
  ...
  try {
    for (var _iterator2 = statsTypes[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      var type = _step2.value;

      assert_true(hasTypes.has(type), 'Expect statsReport to contain stats object of type ' + type);
    }
  } catch (err) {
...
function findStatsFromReport(statsReport, predicate, message) {
  ...
  try {
    for (var _iterator3 = statsReport.values()[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
...
function getRequiredStats(statsReport, type) {
  ...
  try {
    for (var _iterator4 = statsReport.values()[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
...
function getStatsById(statsReport, statsId) {
  assert_true(statsReport.has(statsId), 'Expect stats report to have stats object with id ' + statsId);
...
function validateIdField(statsReport, stats, field, type) {
  ...
  var linkedStats = getStatsById(statsReport, stats[field]);
  assert_equals(linkedStats.type, type, 'Expect linked stats object to have type ' + type);
}
...
function validateMediaStreamStats(statsReport, stats) {
  ...
  try {
    for (var _iterator5 = stats.trackIds[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
      var trackId = _step5.value;

      assert_equals(typeof trackId === 'undefined' ? 'undefined' : _typeof(trackId), 'string', 'Expect trackId elements to be string');

      assert_true(statsReport.has(trackId), 'Expect stats report to have stats object with id ' + trackId);

      var trackStats = statsReport.get(trackId);
      assert_equals(trackStats.type, 'track', 'Expect track stats object to have type \'track\'');
    }
  } catch (err) {
...
function validateMediaStreamTrackStats(statsReport, stats) {
  ...
  assert_optional_enum_field(stats, 'priority', ['very-low', 'low', 'medium', 'high']);
}
...
function validateDataChannelStats(statsReport, stats) {
  ...
  assert_enum_field(stats, 'state', ['connecting', 'open', 'closing', 'closed']);
...
function validateTransportStats(statsReport, stats) {
  ...
  assert_optional_enum_field(stats, 'iceRole', ['controlling', 'controlled']);

  assert_optional_enum_field(stats, 'dtlsState', ['new', 'connecting', 'connected', 'closed', 'failed']);
...
function validateIceCandidateStats(statsReport, stats) {
  ...
  assert_optional_enum_field(stats, 'networkType', ['bluetooth', 'cellular', 'ethernet', 'wifi', 'wimax', 'vpn', 'unknown']);

  assert_string_field(stats, 'ip');
  assert_int_field(stats, 'port');
  assert_string_field(stats, 'protocol');

  assert_enum_field(stats, 'candidateType', ['host', 'srflx', 'prflx', 'relay']);
...
function validateIceCandidatePairStats(statsReport, stats) {
  ...
  assert_enum_field(stats, 'state', ['frozen', 'waiting', 'in-progress', 'failed', 'succeeded']);
