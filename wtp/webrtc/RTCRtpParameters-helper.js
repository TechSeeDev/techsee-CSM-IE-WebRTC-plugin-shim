...
function validateSenderRtpParameters(param) {
  ...
  assert_not_equals(param.transactionId, undefined, 'Expect sender param.transactionId to be set');

  assert_not_equals(param.rtcp.cname, undefined, 'Expect sender param.rtcp.cname to be set');

  assert_not_equals(param.rtcp.reducedSize, undefined, 'Expect sender param.rtcp.reducedSize to be set to either true or false');
}
...
function validateReceiverRtpParameters(param) {
  ...
  assert_equals(param.transactionId, undefined, 'Expect receiver param.transactionId to be unset');

  assert_not_equals(param.rtcp.reducedSize, undefined, 'Expect receiver param.rtcp.reducedSize to be set');

  assert_equals(param.rtcp.cname, undefined, 'Expect receiver param.rtcp.cname to be unset');

  assert_equals(param.degradationPreference, undefined, 'Expect receiver param.degradationPreference to be unset');
}
...
function validateRtpParameters(param) {
  ...
  try {
    for (var _iterator = param.encodings[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
  ...
  try {
    for (var _iterator2 = param.headerExtensions[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
  ...
  try {
    for (var _iterator3 = param.codecs[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
  ...
  assert_optional_enum_field(param, 'degradationPreference', ['maintain-framerate', 'maintain-resolution', 'balanced']);
}
...
function validateEncodingParameters(encoding) {
  ...
  assert_optional_enum_field(encoding, 'dtx', ['disabled', 'enabled']);

  assert_optional_boolean_field(encoding, 'active');
  assert_optional_enum_field(encoding, 'priority', ['very-low', 'low', 'medium', 'high']);
